...
### Fixed
- **`nexus-headroom-intercept` v0.5.17 -- live Claude Code MCP responses were `unsupported_shape`, so nothing was compressed; `session_summary` lacked `mode` (Dispatch `1bc7ff92`, nexus-app, release blocker for 0.14.0).**
  1. **Wrong `tool_response` shape assumption.** A live capture from a real Claude Code session (temporary structure-only `PostToolUse` hook on `mcp__nexus__kb_search`) showed that MCP tool responses arrive as a bare array of content blocks (`[{ type: "text", text }]`), not `{ content: [...] }`. Previous tests fed the assumed object shape and missed this. `normalizeClaudeToolResponse()` now accepts the bare array, `{ content: [...] }`, `{ content: string }`, `structuredContent` (as JSON text when no text blocks exist), and a plain string. `updatedToolOutput` mirrors the source shape (bare array in, bare array out). Responses with any non-text block, or `isError: true`, are passed through unchanged (new optional `preserveVerbatim` on the core `InterceptInput`; previously mixed text/image responses would have been replaced by text only). The live shape is now a test fixture.
  2. **Shape diagnostics.** `unsupported_shape` log lines now carry structure-only fields (`responseType`, `responseLength` or `topLevelKeys`, `firstBlockKeys`), never content (new optional `shapeDiagnostics` on the core `InterceptInput`).
  3. **`session_summary` fields.** The Claude Code adapter's summary now includes `session_id`, `mode` (effective gate mode), `requestedMode`, and `downgradeReason`, read from the cached gate state (no network on `Stop`), so nexus-cli no longer prints "unknown mode".

  12 new unit tests (256/256 passing, typecheck clean); E2E smoke test of the built `plugins/nexus-core/hooks/headroom-intercept.mjs` bundle with the real payload shape confirmed a `transform` event and a summary with `mode: "transform"`. Both adapters bumped to v0.5.17 (shared version scheme); `nexus-core` bundles rebuilt.
- **`nexus-headroom-intercept` v0.5.16 -- `core/state-store.ts` object-spread bug corrupted non-object state (Dispatch `0e38cf7b` review, nexus-app).** nexus-app's review of the v0.5.15 session-accumulation fix found a regression: `loadState()` did `{ ...fallback, ...JSON.parse(raw) }` unconditionally, so a bare JSON string state (the persisted `session_id`, e.g. `"s-1"`) spread into `{0:'s',1:'-',2:'1'}` instead of staying a string. This made every `session_id` comparison in `loadMetricsForSession()` evaluate as "changed", so metrics reset to zero on every single `Stop` call and no `session_summary` was ever logged -- the opposite of the v0.5.15 fix's intent. Fixed: `loadState()` now only object-spread-merges when both the fallback and the parsed value are plain objects; otherwise it returns the parsed value as-is (correct for strings, numbers, arrays, null). Added `core/state-store.test.ts` (7 tests against the real filesystem, no mocks) and a new `adapters/claude-code/headroom-intercept/nexus-headroom-intercept.integration.test.ts` (2 tests) that exercises the real `core/state-store.ts` end to end through `handlePostToolUse`/`handleStop` -- the existing unit test file mocks `state-store.ts` with a plain `Map` and would not have caught this class of bug.
- **`nexus-session-guard` v1.1.3 -- Claude Code adapter never recognized Nexus MCP tool calls (Dispatch `0e38cf7b` review, nexus-app).** Same root cause as the headroom-intercept tool-name-mismatch bug: Claude Code sends Nexus MCP tool calls as `mcp__nexus__task_create`, `mcp__nexus__adr_create`, `mcp__nexus__adr_decide`, `mcp__nexus__session_append`, not the OpenCode-style `nexus_*` names this adapter's `PostToolUse` matcher and `DEFAULT_TRIGGER_MCP`/`isTriggerTool()` check expected -- the hook's matcher never fired for these tool names at all. Fixed with the same `normalizeClaudeToolName()` approach added to the Claude Code headroom-intercept adapter (kept local to each adapter; `core/session-guard/logic.ts` stays runtime-neutral), and extended the `PostToolUse` matcher (in both the standalone adapter's README and the `nexus-core` plugin's `hooks/hooks.json`) to include the `mcp__nexus__...` forms alongside the existing `nexus_*` names. New unit tests for the normalization and for `mcp__nexus__task_create`/`mcp__nexus__session_append` triggering the same behavior as their `nexus_*` equivalents.
- **`nexus-core` plugin bundles rebuilt** (`npm run build:nexus-core-plugin`) to pick up both fixes above; `hooks/hooks.json`'s `PostToolUse` matcher for `session-guard` updated to match the README fix.

### Added
- **`nexus-core` v1.0.0 -- Claude Code plugin and `gatewarden-nexus` marketplace (ADR-0117, Dispatch `0e38cf7b` Part 2, nexus-app).** Repo is public, so installs need no GitHub auth. Added `.claude-plugin/marketplace.json` at the repo root (marketplace `gatewarden-nexus`, one plugin `nexus-core`) and `plugins/nexus-core/` containing: `.claude-plugin/plugin.json`; `hooks/hooks.json` wiring all five migrated Claude Code adapters (`session-guard`, `headroom-intercept`, `compaction-plus`, `routing-guard`, `cost-control`) via `${CLAUDE_PLUGIN_ROOT}/hooks/<adapter>.mjs`; committed single-file ESM bundles built with a new `scripts/build-nexus-core-plugin.mjs` (esbuild, `npm run build:nexus-core-plugin`), with `npm run verify:nexus-core-plugin` wired into CI and the release workflow so the committed bundles can never silently drift from source (plugins install via git, so TypeScript source is not directly executable); six general-purpose skills (`adr-review`, `architecture-review`, `code-review`, `documentation-review`, `implementation-plan`, `test-ops`) and four agents (`architect`, `reviewer`, `security-reviewer`, `documentation-reviewer`), written from scratch in a compact, progressive-disclosure style (not dependent on the Nexus MCP server). Also fixed the `PostToolUse` matcher for `headroom-intercept` in both `plugins/nexus-core/hooks/hooks.json` and the standalone adapter's README install snippet: the previous `nexus_.*|headroom_.*` regex never matches Claude Code's actual `mcp__<server>__<tool>` tool names, so the hook was never invoked at all regardless of the internal policy-lookup fix (see the `nexus-headroom-intercept` v0.5.15 entry above); matcher is now `nexus_.*|headroom_.*|mcp__nexus__.*|mcp__nexus-headroom__.*`. See `plugins/nexus-core/README.md` for install instructions and the `nexus-core-vX.Y.Z` immutable-tag release convention.

### Fixed
- **`nexus-headroom-intercept` v0.5.15 -- Claude Code adapter was inert: 3 bugs fixed (Dispatch `0e38cf7b`, nexus-app, release blocker for nexus-app 0.14.0).** Reproduced on staging that the Claude Code adapter never compressed anything and reported no session stats. Three independent bugs, all fixed:
  1. **Tool-name mismatch.** Claude Code passes MCP tools as `mcp__<server>__<tool>` (e.g. `mcp__nexus__kb_memory`, `mcp__nexus-headroom__headroom_retrieve`), but the core policy table only recognizes OpenCode-style names (`nexus_kb_memory`, `headroom_retrieve`), so every Claude Code MCP tool call fell through `lookupPolicy()` as a no-match skip. Added `normalizeClaudeToolName()` in the Claude Code adapter (core stays runtime-neutral) to map `mcp__nexus__X` -> `nexus_X` and `mcp__nexus-headroom__X` -> `X` before policy lookup.
  2. **Wrong credential/config source.** `getNexusConfig()` resolved env, then `opencode.json`, then the global `~/.config/nexus/{config,credentials}.toml` -- but a project linked to staging via project-local `.nexus/config.toml`/`.nexus/credentials.toml` (the `nexus-cli` >= 0.2x default) would silently fall through to the global (often prod) config, since neither of the first two files exist on the Claude Code path. Resolution order in `core/headroom-intercept/config.ts` is now: env, then `.mcp.json` (`mcpServers.nexus.env`, the Claude analogue of `opencode.json`), then `opencode.json`, then project-local `.nexus/config.toml` + `.nexus/credentials.toml`, then global `~/.config/nexus/*`.
  3. **Per-turn instead of per-session stats.** Claude Code fires `Stop` after every assistant turn, not once per session; the adapter's `handleStop()` previously reset metrics to zero after every `Stop`, so `nexus-cli` (which reads the last `session_summary` line) only ever saw the most recent turn's numbers. Metrics now accumulate cumulatively across the whole session and are only reset when the hook-reported `session_id` actually changes (`loadMetricsForSession()`); every `Stop` logs the running cumulative total.

  7 new unit tests (mcp-prefix normalization, cumulative vs. per-turn accumulation, session-id-triggered reset) plus a new `core/headroom-intercept/config.test.ts` (7 tests covering the full resolution order, including the staging-vs-prod precedence fix). Both adapters bumped to v0.5.15 (shared version scheme).

### Added
- **`nexus-cost-control` v1.2.0 — invert Helicone/native precedence to match ADR-0040 (Dispatch `6298a740`, nexus-app).** ADR-0040 (nexus-app, accepted) chose native message/transcript aggregation over a Helicone proxy specifically because it requires no external service, no additional API keys, and no latency overhead; the shipped code in both adapters had drifted back to gating everything on `HELICONE_API_KEY` being configured, with native aggregation only reached as a fallback for empty Helicone responses (introduced during the ADR-C05 core/adapters restructure, with nothing recording the reversal). Concretely, this meant `nexus_cost_summary` returned "not configured" and `handleStop` bailed out entirely whenever Helicone was not configured, even though native token aggregation (added in v1.1.0) was fully capable of running standalone. Fixed: native aggregation (`aggregateOpenCodeUsage` / `aggregateClaudeUsage`) is now the default, always-on, zero-configuration path in both adapters; `HELICONE_API_KEY` is optional, opt-in enrichment, queried in addition when present, and its result is preferred (as `cost_source: "helicone"`, with a real metered `cost_usd`) only when it returns data. The Claude Code adapter's `handleStop` now only bails on missing `NEXUS_API_URL`/`NEXUS_PRIVATE_TOKEN`, not on missing Helicone config. No change to the `cost_source` field shape, the metadata contract, or the Stop-hook debounce/dedup logic -- only which path is primary. `nexus-cost-control` bumped to v1.2.0 in both adapters.
- **`nexus-cost-control` v1.1.0 — token-only entry when Helicone has no data (Dispatch `515186c1`, nexus-app).** Both the OpenCode and Claude Code adapters previously wrote no session entry at all when `queryHeliconeSession` returned `null` (e.g. Claude Max sessions, which talk directly to Anthropic and never pass through the Helicone gateway). Now, when Helicone is configured but returns no data for a session, the adapters fall back to aggregating token counts directly from the runtime's own data (OpenCode's native `AssistantMessage.tokens`, or the Claude Code transcript's `message.usage` blocks) and append a token-only entry with `cost_usd: null` and `cost_source: "runtime"`. `cost_usd` is explicitly `null`, never `0` -- a zero would be indistinguishable from "ran and cost nothing". Helicone-sourced entries now also carry `cost_source: "helicone"` and a `total_messages` field is added alongside the existing `total_requests`. If no assistant messages are found either, no entry is written (unchanged prior behavior). New shared module `core/cost-control/runtime-usage.ts`.

### Fixed
- **`nexus-compaction-plus` Claude Code adapter: `PostCompact` field name bug.** The adapter read a `summary` field from the `PostCompact` hook input, but the correct field (confirmed against the official Claude Code hooks reference, code.claude.com/docs/en/hooks, on 2026-09-20 -- see Dispatch `dc5fdf9a` reply from nexus-app) is `compact_summary`. Every compaction recorded via this adapter would have had an empty "Agent Summary" section. Fixed; `ClaudeHookInput.summary` renamed to `compact_summary`.
- **`nexus-compaction-plus` Claude Code adapter: `PreCompact` context injection was a permanent no-op.** Confirmed `additionalContext` is not a supported output field for `PreCompact` (or `PostCompact`) -- no documented mechanism exists to inject content before compaction in Claude Code at all. `handlePreCompact()` no longer attempts to return an injection payload (it previously returned one that the runtime would have silently discarded); it now only logs detected Nexus state for diagnostics. This is a genuine, permanent capability gap disclosed in `capability-matrix.v1.json`, not an implementation bug to fix later.
- **`nexus-headroom-intercept` Claude Code adapter: confirmed and simplified `PostToolUse` output field.** `hookSpecificOutput.updatedToolOutput` is the correct field to replace tool output (confirmed 2026-09-20); the previously also-emitted `additionalContext` only adds supplementary text and does not replace the result, so it has been removed. Verified this plugin's policy table never targets built-in tools (Bash/Read/etc., which require preserving their native output schema or the replacement is silently ignored) -- only `nexus_`/`headroom_`-prefixed MCP tool output, which is not schema-validated -- so the `full` capability-matrix rating is now unconditionally confirmed rather than caveated.

### Changed
- **`nexus-session-guard` Claude Code adapter: confirmed correct, no changes.** `hookSpecificOutput.additionalContext` on both `PostToolUse` and `UserPromptSubmit` is verified as the correct field per the official hooks reference (2026-09-20) -- this adapter's original implementation already matched the documented pattern.
- **`routing-guard` Claude Code adapter: fixed to consume the real nexus-cli-generated catalog shape.** See prior entry below; `core/routing-guard/nexus-cli-catalog.ts` added.
- Re-ingested `capability-matrix.v1.json` (Nexus KB `ingest_item 14e7bd6d`) twice in this pass: once reflecting what was actually built in Track B2, and again reflecting the hook-schema verification findings above (compaction-plus bug/gap, headroom-intercept field confirmation, session-guard confirmation, and an empirical hash-scheme comparison against the standalone Headroom MCP server's `headroom_retrieve` for the `custom_tools` entry -- same SHA-256 algorithm, different truncation length, likely separate physical stores; documented as not practical to reconcile, recommending a small nexus-cli-spawned local MCP server instead).

### Changed
- **`routing-guard` Claude Code adapter: switched to the real nexus-cli-generated catalog shape.** Following up on Dispatch `dc5fdf9a`, nexus-app confirmed live against `nexus-cli` v0.17.2 that `nexus pull`/`nexus init` writes `.nexus/generated/routing-catalog.json` (`{ model_routes: [...] }`) and `.nexus/generated/agent-routing.json` (`{ actors: [...] }`), not the plugin's internal `ProviderCatalog`/`AgentInfo[]` shape assumed in the first pass. Added `core/routing-guard/nexus-cli-catalog.ts` to transform the real shape (grouping `model_routes` by provider, resolving each actor's `model_profile_id` against `route_alias`) into the shape `detectRoutingWarnings()` expects; the internal shape is still accepted as a fallback for manual overrides. The Claude Code adapter now defaults to the real file paths (env vars only needed to override); flagged in code comments that the `model_profile_id` <-> `route_alias` linkage assumption is not yet explicitly confirmed by nexus-app. 4 new unit tests for the transform, 2 new adapter-level integration tests. Also confirmed (not a bug): nexus-cli only injects the adapter's env vars into `.claude/settings.json` on first-creation, so pre-existing test projects need `nexus pull --force` after deleting that file to pick up the new defaults -- now less relevant since the adapter finds the files itself without needing the env vars.

### Changed
- **ADR-C05 Track B2 complete: `nexus-cost-control` migrated to `core/` + `adapters/opencode/` + `adapters/claude-code/`.** This is the last plugin in the Track B2 restructure pass (`600-attribution-headers` remains explicitly excluded per ADR-C08). Credential resolution (Nexus + Helicone), state extraction, the Helicone API client, cost-summary formatting, and the Nexus API call extracted to `core/cost-control/*.ts` (types, config, state, helicone, format, api), shared verbatim between both adapters. `200-cost-control/` removed; OpenCode adapter now lives at `adapters/opencode/cost-control/` (behavior unchanged, 6 tests passing), and a new Claude Code adapter (`Stop` hook) lives at `adapters/claude-code/cost-control/` (`partial` parity per `capability-matrix.v1.json`, 7 new tests). Claude Code has no idle-detection event, so cost recording happens around `Stop` instead of `session.idle`, on a different cadence -- disclosed, not silently assumed equivalent. The on-demand `nexus_cost_summary` tool is not implemented as a Claude Code hook (`custom_tools` = `none` for Claude Code per the capability matrix) and needs a Nexus-MCP-server-side follow-up, same gap as the headroom-intercept retrieval tool. 17 new unit tests for the shared core modules. See Dispatch `dc5fdf9a` (NEXUS-APP) and task `554040bb`. With this plugin migrated, ADR-C05's per-plugin `core/`+`adapters/` restructure is structurally complete for the 5 in-scope plugins; several follow-ups remain outside this repo's scope (Nexus MCP server retrieval tool for headroom-intercept and cost-summary, live Claude Code verification for compaction-plus interactive/headless parity and PostToolUse output field names, and `nexus pull`/`init` support for routing-guard's catalog files) -- see each Claude Code adapter's README.
- **ADR-C05 Track B2: `nexus-routing-guard` migrated to `core/` + `adapters/opencode/` + `adapters/claude-code/`.** Pure divergence-detection function (`detectRoutingWarnings`) and banner/context formatters extracted to `core/routing-guard/*.ts` (types, detect, format), shared verbatim between both adapters. `500-routing-guard/` removed; OpenCode adapter now lives at `adapters/opencode/routing-guard/` (behavior unchanged, 16 tests passing), and a new Claude Code adapter (`SessionStart` hook) lives at `adapters/claude-code/routing-guard/` (`partial` parity per `capability-matrix.v1.json`, 5 new tests). No continuous mid-session validation exists in Claude Code (no equivalent to `experimental.chat.system.transform`) -- this gap is disclosed, not worked around, per explicit instruction in Dispatch `dc5fdf9a`. The Claude Code adapter also has no equivalent to OpenCode's `client.config.providers()`/`client.app.agents()` live SDK query, so it requires the provider catalog and agent routing table as JSON files supplied via env vars -- flagged as needing `nexus pull`/`nexus init` support for Claude Code projects, which is outside this repo's scope. 7 new unit tests for the shared core modules. See Dispatch `dc5fdf9a` (NEXUS-APP) and task `554040bb`. The remaining plugin (`200-cost-control`) will follow the same migration pattern; `600-attribution-headers` is explicitly excluded per ADR-C08.
- **ADR-C05 Track B2: `nexus-compaction-plus` migrated to `core/` + `adapters/opencode/` + `adapters/claude-code/`.** State extraction (`extractNexusState`, tool-call driven), context building (`buildNexusContext`), post-compaction text cleanup, and the Nexus API call extracted to `core/compaction-plus/*.ts` (types, config, state, api), shared verbatim between both adapters via a runtime-agnostic `NormalizedToolCall[]` shape. `100-compaction-plus/` removed; OpenCode adapter now lives at `adapters/opencode/compaction-plus/` (behavior unchanged, 11 tests passing), and a new Claude Code adapter (`PreCompact` + `PostCompact` hooks) lives at `adapters/claude-code/compaction-plus/` (`partial` parity per `capability-matrix.v1.json`, 10 new tests, transcript JSONL parsing for tool_use/tool_result pairing). Interactive vs. headless (`claude -p`) hook behavior parity is explicitly flagged as unverified and requires an integration test before claiming full parity, per the dispatch's own guidance. 14 new unit tests for the shared core modules. See Dispatch `dc5fdf9a` (NEXUS-APP) and task `554040bb`. Remaining plugins (`500-routing-guard`, `200-cost-control`) will follow the same migration pattern; `600-attribution-headers` is explicitly excluded per ADR-C08.
- **ADR-C05 Track B2: `nexus-headroom-intercept` migrated to `core/` + `adapters/opencode/` + `adapters/claude-code/`.** Policy table, compression profiles, disk cache (`OriginalStore`), structured JSONL logger, and Nexus credential/project-context resolution extracted to `core/headroom-intercept/*` (types.ts, policies.ts, compression.ts, store.ts, structured-logger.ts, config.ts, engine.ts), shared verbatim between both adapters. `300-headroom-intercept/` removed; OpenCode adapter now lives at `adapters/opencode/headroom-intercept/` (behavior unchanged, 18 tests passing), and a new Claude Code adapter (`PostToolUse` + `Stop` hooks) lives at `adapters/claude-code/headroom-intercept/` (`full` parity target per `capability-matrix.v1.json`, 11 new tests). The Claude Code adapter caches its project/credential gate to disk with a 10-minute TTL instead of a live network preflight per tool call (Claude Code spawns a fresh process per hook event, unlike OpenCode's long-lived plugin instance), and does not implement the `nexus_headroom_intercept_retrieve` retrieval tool locally (`custom_tools` = `none` for Claude Code per the capability matrix) -- flagged as a follow-up requiring an MCP-side retrieval tool on the Nexus MCP server. 27 new unit tests for the shared core modules. Exact `PostToolUse` output field name (`updatedToolOutput` vs. `additionalContext`) flagged for verification against current Claude Code docs before production use. See Dispatch `dc5fdf9a` (NEXUS-APP) and task `554040bb`. Remaining plugins (`100-compaction-plus`, `500-routing-guard`, `200-cost-control`) will follow the same migration pattern; `600-attribution-headers` is explicitly excluded per ADR-C08.
- **ADR-C05 Track B2 pilot: `nexus-session-guard` migrated to `core/` + `adapters/opencode/` + `adapters/claude-code/`.** Runtime-agnostic trigger-detection/state-machine logic extracted to `core/session-guard/logic.ts`; the file-based logger extracted to `core/logger.ts` (shared, previously duplicated per-plugin); a new `core/state-store.ts` provides JSON-file state persistence for adapters whose runtime invokes hooks as short-lived processes. `400-session-guard/` removed; OpenCode adapter now lives at `adapters/opencode/session-guard/`, and a new Claude Code adapter (`PostToolUse` + `UserPromptSubmit` hooks) lives at `adapters/claude-code/session-guard/`. Behavior unchanged for the OpenCode adapter (18 tests, all passing). Claude Code adapter is new (`partial` parity per `capability-matrix.v1.json`; exact hook I/O field names flagged for verification against current docs before production use). See Dispatch `dc5fdf9a` (NEXUS-APP) and task `554040bb`. Remaining plugins (`300-headroom-intercept`, `100-compaction-plus`, `500-routing-guard`, `200-cost-control`) will follow the same migration pattern; `600-attribution-headers` is explicitly excluded per ADR-C08.
